use crate::app::{EmbossArgs, OutputFormat};
use crate::output;
...
pub fn run(args: &EmbossArgs, format: OutputFormat, dry_run: bool, show_schema: bool) -> i32 {
    if show_schema {
        let s = EmbossOp::schema();
    ...
    let input = match &args.input {
        ...
        None => {
            let err = PanimgError::InvalidArgument {
                ...
                suggestion: "usage: panimg emboss <input> -o <output>".into(),
            };
    ...
    if dry_run {
        ...
        output::print_output(
            format,
            &format!("Would emboss {} → {}", input, output_path_str),
            &plan,
        );
        return 0;
    }
    ...
    output::print_output(
        format,
        &format!("Emboss {} → {}", result.input, result.output),
        &result,
    );
